...
def scale_eyes(data,method='scale_to_avg'):
    ...
    # TODO: Remove duplicate functionality with marslab.concat.xcam
    if not all(data['L1'].values) or not all(data['R1'].values): # shared filters don't exist
        return data
    if method == 'scale_to_left':
        ...
        for i in range(len(data.index)):
            ...
            for k in data.keys():
                if ('R' in k) and (not 'ERR' in k) and (not 'L0' in k) and (not 'RMS' in k) and len(k) <= 3:
        ...
        for i in range(len(data.index)):
            ...
            right_scale = data.iloc[i][['L1', 'R1']].mean() / data.iloc[i]['R1']
            for k in data.keys():
                if ('R' in k) and (not 'ERR' in k) and (not 'L0' in k) and (not 'RMS' in k) and len(k) <= 3:
                    # Scale R to L in place
                    #data.iloc[i][k] = data.iloc[i][k] * right_scale # bad
                    data.loc[i,k] = data.iloc[i][k] * right_scale
                elif ('L' in k) and (not 'ERR' in k) and (not 'R0' in k) and (not 'RMS' in k) and len(k) <= 3:
                    #data.iloc[i][k] = data.iloc[i][k] * left_scale # bad
                    data.loc[i,k] = data.iloc[i][k] * left_scale
    return data

def merspect_to_marslab(spectra_fn, # a MERspect exported spectra file, unmodified
    ...
    columns = {  # Some of these parameters are only meaningful to WWU Marslab
        "MCAM": ["SOL", "SEQ_ID", "INSTRUMENT", "COLOR", "FEATURE", "FORMATION", "MEMBER", "FLOAT",
                 "L2", "L2_ERR", "R2", "R2_ERR", "L0B", "L0B_ERR", "R0B", "R0B_ERR",
                 "L1", "L1_ERR", "R1", "R1_ERR", "R0G", "R0G_ERR", "L0G", "L0G_ERR",
                 "R0R", "R0R_ERR", "L0R", "L0R_ERR",
                 "L4", "L4_ERR", "L3", "L3_ERR", "R3", "R3_ERR", "L5", "L5_ERR",
                 "R4", "R4_ERR", "R5", "R5_ERR", "L6", "L6_ERR", "R6", "R6_ERR",
                 ],  # Some of these parameters are only meaningful to WWU Marslab
        "ZCAM": ["SOL", "SEQ_ID", "INSTRUMENT", "COLOR", "FEATURE", "FORMATION", "MEMBER", "FLOAT",
                 "L6", "L6_ERR", "L0B", "L0B_ERR", "R0B", "R0B_ERR", "L5", "L5_ERR",
                 "L0G", "L0G_ERR", "R0G", "R0G_ERR", "L4", "L4_ERR", "L0R", "L0R_ERR", "R0R", "R0R_ERR",
                 "L3", "L3_ERR", "L2", "L2_ERR", "L1", "L1_ERR", "R1", "R1_ERR", "R2", "R2_ERR",
                 "R3", "R3_ERR", "R4", "R4_ERR", "R5", "R5_ERR", "R6", "R6_ERR",
                 ],
    }[instrument]
    data = pd.DataFrame(columns=columns)

    # Generate the index column --- 'COLOR'
    colors = [' '.join(k.split(' ')[:-2]) for k in csv.keys() if k.endswith('Mean Value')]
    data["COLOR"] = colors
...
def convert_for_plot(spectra_fn,instrument='ZCAM',color_to_feature={},scale_method='scale_to_avg'):
    ...
    for fn in spectra_fn:
        try:
            ...
            marslab_data = merspect_to_marslab(fn,
